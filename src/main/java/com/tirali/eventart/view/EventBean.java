...
public class EventBean implements Serializable
{
   ...
   public void retrieve()
   {
      ...
      else
      {
       ...
       CriteriaBuilder builder = this.entityManager.getCriteriaBuilder();
 		...
 		TypedQuery<User> userQuery = this.entityManager.createQuery(userCriteria
 				.select(userRoot).where(getUserSearchPredicates(userRoot)));
 		userQuery.setFirstResult(this.page * getPageSize()).setMaxResults(
 				getPageSize());
 		this.allUsers = userQuery.getResultList();
   }
   ...
   private Predicate[] getSearchPredicates(Root<Event> root)
   {
      ...
      String theme = this.example.getTheme();
      if (theme != null && !"".equals(theme))
      {
         predicatesList.add(builder.like(root.<String> get("theme"), '%' + theme + '%'));
      }
      String place = this.example.getPlace();
      if (place != null && !"".equals(place))
      {
         predicatesList.add(builder.like(root.<String> get("place"), '%' + place + '%'));
      }

      return predicatesList.toArray(new Predicate[predicatesList.size()]);
   }
   
   private Predicate[] getUserSearchPredicates(Root<User> root) {

		CriteriaBuilder builder = this.entityManager.getCriteriaBuilder();
		List<Predicate> predicatesList = new ArrayList<Predicate>();

		//empty since we want all
